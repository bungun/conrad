...
x_grid, y_grid, regions = simple_structures(m_grid, n_grid, xlim, ylim)
...
if CALC_AMAT:
	...
	if PLOT_LINES:
		...
		struct_kw = simple_colormap()
		struct_tuple = (x_grid, y_grid, regions)
	...
	if SAVE_AMAT:
		np.save("data/A_cardioid_rot_{0}x{1}-grid_{2}-angles_{3}-bundles.npy".format(m_grid, n_grid, n_angle, n_bundle), A)
