...
class ConradDatabaseBaseTestCase(ConradTestCase):
	...
	def test_raw_data_to_entry(self):
		...
		for constructor in self.entry_types:
			...
			self.assertTrue( type(obj_out) == constructor )
	...
	def test_key_methods(self):
		...
		for t in self.entry_types:
			...
			self.assertTrue( key_out == key_expect )
			...
			self.assertTrue( type_out == type_expect )
...
class LocalPythonDatabaseTestCase(ConradTestCase):
	...
	def test_lpdb_set(self):
		...
		self.assertTrue( lpdb.set('garbage key', None) is None )
	...
	def test_lpdb_set_next(self):
		...
		self.assertTrue( lpdb.set_next(None) is None )
		with self.assertRaises(TypeError):
			lpdb.set_next({})
	...
	def test_lpdb_get(self):
		...
		self.assertTrue( lpdb.get(None) is None )

		idx_frag = 0
		for entry_type in self.entry_types:
			et_instance = entry_type()
			self.assertTrue( isinstance(lpdb.get(et_instance), entry_type) )
			...
			for i in xrange(10):
				...
				self.assertTrue( isinstance(lpdb.get(key), entry_type) )
				with self.assertRaises(KeyError):
	...
	def test_lpdb_clear_log(self):
		...
		self.assertTrue( len(lpdb.logged_entries) == 0 )
	...
	def test_lpdb_entry_flattening(self):
		...
		ve.flat_dictionary
		self.assertTrue( len(lpdb.logged_entries) == n_logged )
	...
	def test_lpdb_dump_dictionary(self):
		...
		self.assertTrue( isinstance(db_dict, dict) )
		self.assertTrue( 'data_fragments' in db_dict )
		self.assertTrue( len(db_dict['data_fragments']) > 0 )
		key_expect = CONRAD_DB_ENTRY_PREFIXES[VectorEntry] + str(0)
		self.assertTrue( key_expect in db_dict['data_fragments'] )
		self.assertTrue(
				isinstance(db_dict['data_fragments'][key_expect], dict) )

		self.assertTrue( 'solutions' in db_dict )
		self.assertTrue( len(db_dict['solutions']) == 0 )

		lpdb.set_next(SolutionEntry(x=VectorEntry()))
		db_dict = lpdb.dump_to_dictionary()

		self.assertTrue( len(db_dict['solutions']) == 1 )
		key_expect = CONRAD_DB_ENTRY_PREFIXES[SolutionEntry] + str(0)
		self.assertTrue( key_expect in db_dict['solutions'] )
		self.assertTrue( isinstance(db_dict['solutions'][key_expect], dict) )
		sol_entry = db_dict['solutions'][key_expect]

		self.assertTrue( len(db_dict['data_fragments']) == 2 )
		key_expect = CONRAD_DB_ENTRY_PREFIXES[VectorEntry] + str(1)
		self.assertTrue( 'x' in sol_entry )
		self.assertTrue( sol_entry['x'] == key_expect )
	...
	def test_lpdb_ingest_dictionary(self):
		...
		self.assertTrue( isinstance(lpdb2.get('data_fragment.0'), VectorEntry) )
		self.assertTrue( isinstance(lpdb2.get('data_fragment.1'), VectorEntry) )
		self.assertTrue( isinstance(lpdb2.get('solution.0'), SolutionEntry) )
