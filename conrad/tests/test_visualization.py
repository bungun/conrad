...
try:
	...
	class VisualizationTestCase(ConradTestCase):
		...
		def test_panels_to_cols(self):
			...
			self.assertTrue( d.panels_to_cols(1) == 1 )
			self.assertTrue( d.panels_to_cols(2) == 2 )
			self.assertTrue( d.panels_to_cols(3) == 2 )
			self.assertTrue( d.panels_to_cols(4) == 2 )
			self.assertTrue( d.panels_to_cols(5) == 3 )
			self.assertTrue( d.panels_to_cols(6) == 3 )
			self.assertTrue( d.panels_to_cols(7) == 4 )
			self.assertTrue( d.panels_to_cols(8) == 4 )
			self.assertTrue( d.panels_to_cols(12) == 4 )
			self.assertTrue( d.panels_to_cols(15) == 4 )
			self.assertTrue( d.panels_to_cols(25) == 4 )
		...
		def test_dvh_plot_init(self):
			...
			self.assertFalse( isinstance(d.figure, Figure) )
		...
		def test_dvh_plot_build(self):
			d = DVHPlot(self.panels, self.names)
			if d is None:
				return

			d.build()
			self.assertTrue( isinstance(d.figure, Figure) )
		...
		def test_case_plotter_plot(self):
			...
			self.assertTrue( isinstance(cp.dvh_plot.figure, Figure) )
