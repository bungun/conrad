...
Attributes:
	PLOTTING_INSTALLED (bool): True if `matplotlib` is not available. If
		so, `DVHPlot` and `CasePlotter` types are replaced with lambdas
		that match the initialization argument signature and each yield
		`None` instead.

		If `matplotlib` *is* available, module defines `DVHPlot` and
		`CasePlotter` types normally.

		This switch allows CONRAD package to install, load and operate
		without Python plotting capabilities, and exempts matplotlib
		from being a load-time requirement.
...
def panels_to_cols(n_panels):
	...
	Args:
		n_panels, int: number of subplot panels.
...
if not PLOTTING_INSTALLED:
	...
	class DVHPlot(object):
		...
		Figure contains `__n_structures` DVH curves distributed among
		(`__cols` by `__rows`) subplots. This can be adjusted
		...
		def __init__(self, panels_by_structure, names_by_structure):
			"""
			Initialize `DVHPlot` object.

			Initialize a `matplotlib.Figure` object as a blank canvas.
			Private dictionaries track panel (subplot index), series
			name, and series color assignments, all keyed by structure
		...
		def autoset_series_colors(self, structure_order_dict=None,
								  colormap=None):
			"""
			Set series colors to a given/default `LinearSegmentedColormap`.
			...
			Args:
				...
				colormap (:obj:`str`, optional) Assumed to be valid
					`matplotlib.pyplot` colormap name.
		...
		def plot(self, plot_data, show=False, clear=True, xmax=None, legend=False,
				 ...
				 **options):
			"""
			Plot `plot_data` to the object's `matplotlib.Figure`.

			Args:
				plot_data (:obj:`dict`) with following organization
					scheme::
				...
				show (bool, optional): Show `matplotlib.Figure` canvas
					after `plot_data` elements are drawn.
				clear (bool, optional): Clear `matplotlib.Figure` canvas
					reset before `plot_data` elements are drawn.
				xmax (float, optional): . Upper limit for x-axis set to
					this value if provided. Otherwise, upper limit set
					to 110% of largest dose encountered in `plot_data`.
				legend (bool, optional): Enable legend in
					`matplotlib.Figure`.
				title (:obj:`str`, optional): Contents drawn as title of
					`matplotlib.Figure`.
				large_markers (bool, optional): Draw dose volume
					constraints with larger size markers.
				suppress_constraints (bool, optional): Suppress
					rendering of dose volume constraints.
				suppress_xticks (bool, optional): Suppress drawing of
					x-axis ticks.
				suppress_yticks (bool, optional): Suppress drawing of
					y-axis ticks.
				suppress_xlabel (bool, optional): Suppress drawing of
					x-axis label.
		...
		def save(self, filepath, overwrite=True, verbose=False):
			...
			Args:
				...
				overwrite (bool):, Allow overwrite of file at `filepath`
					if True.
				verbose (bool):, print confirmation of save if True
			...
			Raises:
				ValueError: If `filepath` does not exist *or* is an
					existing file and flag `overwrite` == False.
		...
		def __del__(self):
			""" Close object's `matplotlib.Figure` on object deletion."""
			plt.close(self.fig)

	class CasePlotter(object):
		"""
		Wrap `DVHPlot` for visualizing treatment plan data.

		Attributes:
			dvh_plot (`DVHPlot`): Dose volume histogram plot.
		...
		def __init__(self, case):
			"""
			Initialize `CasePlotter`.

			Use structure information from `case` to initialize a
			`DVHPlot` object with the names and labels of each structure
			associated with the case.

			Args:
				case (`Case`): Treatment planning case to use as
					basis for configuring object's `DVHPlot`
		...
		def set_display_groups(self, grouping='together', group_list=None):
			...
			Raises:
				TypeError: If `grouping` is not a :obj:`str` or members
					of `group_list` are not each a :obj:`tuple`.
		...
		def plot(self, data, second_pass=False, show=False, clear=True,
			...
			Args:
				...
				second_pass (bool, optional): Plot data from second
					planning pass when Trye and `data` is a `RunRecord`.
				show (bool, optional): Show figure after drawing.
				clear (bool, optional): Clear figure before rendering
					data in `data`.
			...
			Raises:
				TypeError: If `subset` is specified but not a list or
					tuple.
